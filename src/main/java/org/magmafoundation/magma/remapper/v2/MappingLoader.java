...
public class MappingLoader {
	...
	private static void initMappings() {
		...
		try {
			...
			jarMappingGlobal.loadMappings(new BufferedReader(new InputStreamReader(MappingLoader.class.getClassLoader().getResourceAsStream("mappings/nms.srg"))), new MavenShade(relocations), null, false);
	...
	public static JarMapping loadMapping() {
		...
		try {
			...
			try {
				...
				unsafe.putObject(jarMapping, unsafe.objectFieldOffset(fieldPackages), fieldPackages.get(jarMappingGlobal));
				unsafe.putObject(jarMapping, unsafe.objectFieldOffset(fieldClasses), fieldClasses.get(jarMappingGlobal));
				unsafe.putObject(jarMapping, unsafe.objectFieldOffset(fieldFields), fieldFields.get(jarMappingGlobal));
				unsafe.putObject(jarMapping, unsafe.objectFieldOffset(fieldMethods), fieldMethods.get(jarMappingGlobal));
			}catch (Exception e1) {
				e1.printStackTrace();
			}
		}
